...
def build_vgg19(input,reuse=False):
    ...
    print("1_2",net["conv1_2"],net["conv1_2"].get_shape())
    net['pool1']=build_net('pool',net['conv1_2'])
    net['conv2_1']=build_net('conv',net['pool1'],get_weight_bias(vgg_layers,5),name='vgg_conv2_1')
    net['conv2_2']=build_net('conv',net['conv2_1'],get_weight_bias(vgg_layers,7),name='vgg_conv2_2')
    print("2_2",net["conv2_2"],net["conv2_2"].get_shape())
...
def recursive_generator(label,sp):
    ...
    if sp==4:
        input=label
        #input =tf.image.resize_bilinear(input,(sp,sp*2),align_corners=True)
    else:
...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    label2 =tf.image.resize_bilinear(label,(sp,sp*2),align_corners=True)
    ...
    p1=compute_error(vgg_real['conv1_2'],vgg_fake['conv1_2'],tf.image.resize_area(label2,(sp,sp*2)))/1.6
    p2=compute_error(vgg_real['conv2_2'],vgg_fake['conv2_2'],tf.image.resize_area(label2,(sp//2,sp)))/2.3
    p3=compute_error(vgg_real['conv3_2'],vgg_fake['conv3_2'],tf.image.resize_area(label2,(sp//4,sp//2)))/1.8
    p4=compute_error(vgg_real['conv4_2'],vgg_fake['conv4_2'],tf.image.resize_area(label2,(sp//8,sp//4)))/2.8
    p5=compute_error(vgg_real['conv5_2'],vgg_fake['conv5_2'],tf.image.resize_area(label2,(sp//16,sp//8)))*10/0.8#weights lambda are collected at 100th epoch
...
v = []
for vv in variables:
    if not vv.name.startswith("g_4_conv1/weights"):
        print(vv.name)
        v.append(vv)
    ...
    for epoch in range(1,201):
        ...
        for ind in range(NUM_TRAINING_IMAGES,NUM_TRAINING_IMAGES+50):
            if not os.path.isfile("data/cityscapes/Label256Full/%08d.png"%ind):#test label
                continue
            bsemantic = np.load("hmlabels/%06d.npz"%ind)["arr_0"]#training label
            b, h, w, c = bsemantic.shape
            semantic = np.zeros([b, 256, 512, c])
            for i in range(c):
                for j in range(b):
                    semantic[j,:,:,i] = cv2.resize(bsemantic[j,:,:,i], (semantic.shape[2], semantic.shape[1]))
